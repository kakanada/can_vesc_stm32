...
Тип колбэка для приёма произвольных кастомных команд — см.
[Произвольные кастомные команды](#произвольные-кастомные-команды).
...
> **Мотор не реагирует на `SendSpeed`/`SendMechanicalSpeed`, хотя на
> `SendCurrent` реагирует нормально?** Пакет отправляется корректно (сверено
> построчно с прошивкой VESC) — почти наверняка дело в ненастроенном Speed
> PID и/или пороге Minimum ERPM (~900 по умолчанию) в VESC Tool → Motor
> Settings → PID Controllers → Speed Controller. Подробности — у объявления
> `VESC_CAN_SendSpeed` в `motor_vesc.h`.
...
Для собственного протокола сверх официального VESC (например обмен со своим
Lisp-скриптом), когда заранее готовых `VESC_CAN_SendReleaseBrake`/
`VESC_CAN_PACKET_STATUS_7` недостаточно — полностью произвольный код команды
и произвольные данные (0..8 байт) в обе стороны. Как и `SendReleaseBrake`,
не проходит через программную очередь.

> **Внимание**: код команды (`custom_cmd_id`) — то же 8-битное поле, что и у
> штатных команд протокола VESC (`VESC_CAN_PacketId_t`). Если случайно выбрать
> значение, уже занятое официальной командой (например `SET_RPM`) — настоящая
> прошивка вески применит присланные байты именно как эту настоящую команду.
> Выбирайте значение, не входящее в `VESC_CAN_PacketId_t`.

### `HAL_StatusTypeDef VESC_CAN_SendCustomCommand(VESC_Handle_t *h, uint8_t custom_cmd_id, const uint8_t *data, uint8_t len)`
Отправляет один кадр с произвольным кодом команды и данными веске. `len` —
0..8 (классический CAN-кадр), `HAL_ERROR` при превышении. `HAL_BUSY` при
занятом аппаратном буфере, без досылки (как у `SendReleaseBrake`).

### `HAL_StatusTypeDef VESC_CAN_SetCustomCommandCallback(VESC_Handle_t *h, VESC_CustomCommandCallback_t callback)`
Задаёт обработчик, вызываемый при получении от вески кадра, код команды
которого НЕ входит в набор штатных статусов (`VESC_CAN_PACKET_STATUS` ..
`STATUS_7`) — сигнатура `void(*)(VESC_Handle_t *h, uint8_t custom_cmd_id, const uint8_t *data, uint8_t len)`.
`NULL` отключает. **Вызывается из прерывания** — быстрый, неблокирующий код;
`data`/`len` валидны только на время вызова.
...
Досылает отложенные команды весок этой шины по кругу (round-robin). Не
единственный путь досылки — каждый `VESC_CAN_SendXxx` тоже опортунистически
досылает очередь при каждом вызове (см. `motor_vesc.h`), поэтому очередь не
зависает, даже если это прерывание по какой-то причине не пришло.

---

## Диагностика и восстановление шины

Честно: сама по себе загрузка шины корректными кадрами (даже на пределе
пропускной способности) не может "уронить" CAN — единственный механизм,
из-за которого шина полностью отключает ВСЕХ узлов сразу — это Bus-Off,
следствие реальных ошибок кадров (несовпадающий битовый тайминг у узлов,
неисправный/не тот узел на шине, отсутствующий/лишний терминатор, помехи).
Обработчик ниже **обнаруживает и автоматически восстанавливает** периферию
после Bus-Off — FDCAN не выходит из него сам никогда, bxCAN без опции ABOM в
CubeMX тоже. До появления этой секции восстановление после Bus-Off требовало
перезагрузки МК. Подробности — в `motor_vesc.h`.

### `void VESC_CAN_ErrorStatus_Handler(hcan[, ErrorStatusITs])` — вызывается из вашего HAL callback-а
Bus-Off: инкрементирует счётчик и немедленно восстанавливает периферию.
Сигнатура для FDCAN — `(hcan, ErrorStatusITs)`, вызывать из
`HAL_FDCAN_ErrorStatusCallback`; для bxCAN — `(hcan)`, вызывать из
`HAL_CAN_ErrorCallback`. Также считает переполнения Rx FIFO0.

### `uint32_t VESC_CAN_GetBusOffCount(VESC_CAN_HandleTypeDef *hcan)`
Сколько раз шина уходила в Bus-Off (и была восстановлена) с момента старта.
Растущее значение — признак физической/конфигурационной проблемы на шине,
не в библиотеке.

### `uint32_t VESC_CAN_GetRxOverflowCount(VESC_CAN_HandleTypeDef *hcan)`
Сколько раз переполнялся аппаратный Rx FIFO0 (кадры потеряны, программа не
успела их вычитать). В штатной работе должно быть 0.
