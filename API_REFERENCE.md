...
`VESC_CAN_SetLocalId()` для этой шины (иначе `HAL_ERROR`). **[После миграции на can_manager]**
`HAL_BUSY` больше не возвращается — `CANMGR_Send()` ставит пакет в свою программную очередь, функция
возвращает `HAL_OK` заметно чаще, чем раньше (без программной очереди-дедупликатора этой библиотеки
— как и `VESC_CAN_SendReleaseBrake`/`SendCustomCommand`).
...
программную очередь-дедупликатор этой библиотеки — отправляется прямым `CANMGR_Send()`.
**[После миграции на can_manager]** `HAL_BUSY` больше не возвращается — см. изменение поведения у
`VESC_CAN_RequestExists()` выше, тот же принцип.
...
не проходит через программную очередь-дедупликатор, отправляется прямым `CANMGR_Send()`.
...
(переполнена его собственная программная очередь). **[После миграции на can_manager]** `HAL_BUSY`
больше не возвращается (как и у `SendReleaseBrake`).
...
Досылка отложенных команд-дедупликатора этой библиотеки (round-robin между вескими) теперь ЦЕЛИКОМ
опортунистическая — на каждый `VESC_CAN_SendXxx` (см. `motor_vesc.h`), выделенного
"буфер освободился"-обработчика у `motor_vesc` больше нет (это стало заботой `can_manager` для его
СВОЕЙ очереди).
